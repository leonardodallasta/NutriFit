...
export default function StatisticsScreen() {
  ...
  const adicionarAgua = (quantidade: number) => {
    const novoConsumo = Math.max(0, stats.consumoAgua + quantidade);
  ...
  const alterarMeta = (quantidade: number) => {
    const novaMeta = Math.min(5000, Math.max(2500, stats.metaAgua + quantidade));
    ...
    <View style={styles.container}>
      ...
      {loading ? <ActivityIndicator size="large" color="#00A99D"/> : (
        <View style={styles.card}>
          <StatItem label="Água" value={`${stats.consumoAgua / 1000} / ${stats.metaAgua / 1000}L`} progress={stats.metaAgua > 0 ? stats.consumoAgua / stats.metaAgua : 0} color="#3b82f6" openModal={() => setModalVisible(true)} />
        ...
        <View style={styles.modalContainer}>
          <View style={styles.modalContent}>
            ...
            <View style={styles.modalButtons}>
              <TouchableOpacity style={styles.modalButton} onPress={() => adicionarAgua(-100)}><Text style={styles.modalButtonText}>-100mL</Text></TouchableOpacity>
              <TouchableOpacity style={styles.modalButton} onPress={() => adicionarAgua(100)}><Text style={styles.modalButtonText}>+100mL</Text></TouchableOpacity>
            </View>
            <Text style={styles.modalTitle}>Definir Meta</Text>
            <Text style={styles.metaValue}>{(stats.metaAgua || 2500) / 1000} L</Text>
            <View style={styles.modalButtons}>
              <TouchableOpacity style={styles.modalButton} onPress={() => alterarMeta(-250)}><Text style={styles.modalButtonText}>-0.25L</Text></TouchableOpacity>
              <TouchableOpacity style={styles.modalButton} onPress={() => alterarMeta(250)}><Text style={styles.modalButtonText}>+0.25L</Text></TouchableOpacity>
            </View>
